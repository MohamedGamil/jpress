...
class AdminPage extends XboxCore {
    ...
    */
    public function redefine_options($k){
        $remove =   array('_wp_http_referer','xbox_id','xbox_nonce_appbear-settings','xbox-save','xbox-import-field');
        if(is_array($k)){
    ...
    */
    public function after_save_fields( $data, $object_id, $updated_fields = array() ){
            ...
            case 'appbear-settings':
                 ...
                 ***************************************************************/
                    ...
                    */
                    if($data['menu_type']!='sidemenu' && isset($data["bottombar_tabs"]) && !empty($data["bottombar_tabs"])){
                        ...
                        foreach($data['bottombar_tabs'] as $key=> $navigator){
                            ...
                            // $bottombar[$navigator['main']]   =   array(
                            //     'icon'  =>  $navigator['icon'],
                            ...
                            // );
                            // array_push($options['bottomBar']['navigators'], $bottombar);
                        }
                    }
                    ...
                    */
                    ...
                    foreach($data['sections'] as $key => $section){
                        ...
                        if(isset($section["local-section_title"]) && $section["local-section_title"] != 'false')
                        {
                            ...
                            if(isset($section["local-enable_see_all"]) && $section["local-enable_see_all"] != 'false')
                            {
                                $item['seeMore']  =   array(
                                    'name'  =>  $item['title'],
                                    'url'   =>  $item['url']
                    ...
                    */
                    ...
                    $options['styling']['ThemeMode.light']['errorColor']    =   "ff0000";
                    $options['styling']['ThemeMode.light']['successColor']    =   "00ff00";
                    ...
                    if(isset($data['switch_theme_mode']) && $data['switch_theme_mode'] != 'false'){
                        ...
                        $options['styling']['ThemeMode.dark']['errorColor']    =   "ff0000";
                        $options['styling']['ThemeMode.dark']['successColor']    =   "00ff00";
                    }
        ...
        if($change_language == false){
            wp_remote_get("https://appstage.tielabs.com/?edd_action=send_silent_fcm_message&site_url=".$licensedBase);
        }else{
            wp_remote_get("https://appstage.tielabs.com/?edd_action=send_silent_fcm_message&site_url=".$licensedBase."&change_translations=true");
        }
