	...
	public function build_tab_menu(){
		...
		foreach ( $items as $key => $display ){
			...
			if( empty( $show_if ) || empty( $show_if[0] ) ){
				...
				$get_xbox = \Xbox::get($this->field->get_xbox()->id);
				if($field_value = $get_xbox->get_field_value($show_if[0])){
					$value = '';
					$operator = '==';
					if( count( $show_if ) == 2 ){
			...
			if( empty( $hide_if ) || empty( $hide_if[0] ) ){
				...
				$get_xbox = \Xbox::get($this->field->get_xbox()->id);
				if($field_value = $get_xbox->get_field_value($hide_if[0])){
					$value = '';
					$operator = '==';
					if( count( $hide_if ) == 2 ){
	...
	*/
	public function visibility_row_class(){
		...
		if( empty( $show_if ) || empty( $show_if[0] ) ){
			...
			$get_xbox = \Xbox::get($this->field->get_xbox()->id);
			if($get_xbox->get_field_value($show_if[0]) == ''){
				$parent = $this->field->get_parent();
				$field = $parent->get_field( $show_if[0] );
				if($field != null)
				$field_value = $field->get_value();
			}else{
				$field_value = $get_xbox->get_field_value($show_if[0]);
			}
		...
		if( empty( $hide_if ) || empty( $hide_if[0] ) ){
			...
			$get_xbox = \Xbox::get($this->field->get_xbox()->id);
			if($get_xbox->get_field_value($hide_if[0]) == ''){
				$parent = $this->field->get_parent();
				$field = $parent->get_field( $hide_if[0] );
				$field_value = $field->get_value();
			}else{
				$field_value = $get_xbox->get_field_value($hide_if[0]);
			}
			if($field_value){
				$value = '';
				$operator = '==';
