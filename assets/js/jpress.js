  ...
  jpress.init_dropdown = function ($selector) {
    ...
    $selector.each(function (index, el) {
      ...
      if (max_selections > 1 && $(el).hasClass('multiple')) {
        ...
        $(el).dropdownJPress('set selected', value.split(','));
      } else {
        $(el).dropdownJPress();
      }
    });
  ...
  jpress.set_field_value = function ($field, value, extra_value, update_initial_values) {
    ...
    switch (type) {
      ...
      case 'select':
        ...
        $dropdown.dropdownJPress('clear');
        if (max_selections > 1 && $dropdown.hasClass('multiple')) {
          $dropdown.dropdownJPress('set selected', value.split(','));
        } else {
          $dropdown.dropdownJPress('set selected', value);
        }
        break;
