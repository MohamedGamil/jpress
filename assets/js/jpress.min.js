...
window.APPBEAR = (function (window, document, $) {
  ...
  jpress.load_icons_for_icon_selector = function (event) {
    ...
    $(document).on('click', '.jpress-icons-wrap .jpress-item-icon-selector', function (event) {
      ...
      var $container = $field.find('.jpress-icons-wrap');
      var options = $container.data('options');
      $(this).addClass(options.active_class).siblings().removeClass(options.active_class);
      $field.find('input.jpress-element').val($(this).data('value')).trigger('change');
      $field.find('.jpress-icon-active').html($(this).html());
    });
  };
      ...
      if (media.multiple) {
        ...
        if (obj.type == 'image') {
          ...
          if (media.field_id == control_img_id) {
            jpress.synchronize_selector_preview_image('.jpress-control-image', media.$wrap_preview, 'add', obj.url);
          }
          //Sincronizar con otros elementos
          jpress.synchronize_selector_preview_image('', media.$wrap_preview, 'add', obj.url);
        }
      }
    });
  ...
  jpress.remove_preview_item = function (event) {
    ...
    if (!multiple && $btn.closest('.jpress-preview-item').hasClass('jpress-preview-image')) {
      if (field_id == control_data_img) {
        jpress.synchronize_selector_preview_image('.jpress-control-image', $wrap_preview, 'remove', '');
      }
      jpress.synchronize_selector_preview_image('', $wrap_preview, 'remove', '');
    }
  ...
  jpress.synchronize_selector_preview_image = function (selectors, $wrap_preview, action, value) {
...
APPBEAR.events = (function (window, document, $) {
  ...
  jpress_events.on_change_file = function ($jpress) {
    $jpress.on('change', '.jpress-type-file .jpress-element', function () {
      ...
      if (jpress.is_image_file(value) && !multiple) {
        var $wrap_preview = $(this).closest('.jpress-field').find('.jpress-wrap-preview').first();
  ...
  jpress_events.on_change_text = function ($jpress) {
    $jpress.on('input', '.jpress-type-text .jpress-element', function () {
      ...
      var $helper = $input.next('.jpress-field-helper');
  ...
  jpress_events.on_change_wp_editor = function ($jpress) {
    var $wp_editors = $jpress.find('.jpress-type-wp_editor textarea.wp-editor-area');
  ...
  jpress_events.show_hide_row = function ($el, field_value, type) {
    var prefix = $el.closest('.jpress').data('prefix');
    var $row_changed = $el.closest('.jpress-row');
    var value = '';
    var operator = '==';
    var $rows = $row_changed.siblings('.jpress-row');
    var $group_item = $row_changed.closest('.jpress-group-item');
      ...
      if (check_show) {
        if ($.isArray(show_if[0])) {
          ...
          if ($.inArray(operator, ['==', '!=', '>', '>=', '<', '<=']) > -1) {
            show = jpress.compare_values_by_operator(field_value, operator, value);
