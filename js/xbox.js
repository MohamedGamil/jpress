...
window.XBOX = (function (window, document, $) {
  ...
  xbox.load_icons_for_icon_selector = function (event) {
    ...
    //   $('.xbox-icons-wrap').removeClass('d-block');
    //   $('.xbox-search-icon').removeClass('d-block');
    // });
    $(document).mouseup(function(e) 
    {
        var container = $(".xbox-type-icon_selector .xbox-field .xbox-search-icon");
        ...
        var container = $(".xbox-type-icon_selector .xbox-field .xbox-icons-wrap");
    ...
    $(document).on('click', '.xbox-icon-actions', function (event) {
      event.stopPropagation();
      $(this).closest('.xbox-field').find('.xbox-icons-wrap').addClass('d-block');
      $(this).closest('.xbox-field').find('.xbox-search-icon').addClass('d-block');
      return false; 
    });
    ...
    $(document).on('click', '.xbox-icons-wrap .xbox-item-icon-selector', function (event) {
      ...
      $field.find('.xbox-icons-wrap').css('style', 'display:block  !important');

      $(".xbox-type-icon_selector .xbox-field .xbox-icons-wrap,.xbox-type-icon_selector .xbox-field .xbox-search-icon").removeClass('d-block');
    });
  };
      ...
      if (media.multiple) {
        ...
        if (obj.type == 'image') {
          ...
          if (media.field_id == control_img_id) {
            xbox.synchronize_selector_preview_image('.xbox-control-image', media.$wrap_preview, 'add', obj.url, control_img_id);
          }
          //Sincronizar con otros elementos
          xbox.synchronize_selector_preview_image('', media.$wrap_preview, 'add', obj.url, control_img_id);
        }
      }
    });
  ...
  xbox.remove_preview_item = function (event) {
    ...
    if (!multiple && $btn.closest('.xbox-preview-item').hasClass('xbox-preview-image')) {
      if (field_id == control_data_img) {
        xbox.synchronize_selector_preview_image('.xbox-control-image', $wrap_preview, 'remove', '', control_data_img);
      }else{
        xbox.synchronize_selector_preview_image('', $wrap_preview, 'remove', '', control_data_img);
      }
    }
  ...
  xbox.synchronize_selector_preview_image = function (selectors, $wrap_preview, action, value, control_img_id = null) {
    ...
    if (!is_empty(selectors)) {
      ...
      $.each(selectors, function (index, selector) {
        ...
        if ($element.closest('.xbox-type-group').length) {
          if ($element.closest('.xbox-type-group').find('.xbox-group-control').length) {
        ...
        if ($element.is('img')) {
          ...
          $element.closest('.xbox-type-group').find('.xbox-group-control[data-image-field-id="'+control_img_id+'"] .xbox-group-control-item.xbox-active').find(selector).fadeOut(300, function () {
            if ($element.closest('.xbox-type-group').find('.xbox-group-control').length) {
              if(control_img_id != null){
                $element.closest('.xbox-type-group').find('.xbox-group-control[data-image-field-id="'+control_img_id+'"] .xbox-group-control-item.xbox-active').find(selector).css('background-image', 'url(' + value + ')');
              }else{
...
(function ($) {
  $('.xbox .xbox-type-icon_selector .xbox-icon-active.xbox-item-icon-selector').on('click', function(){
    alert('test');
    $('.xbox .xbox-type-icon_selector .xbox-search-icon, .xbox .xbox-type-icon_selector .xbox-icons-wrap').css('style', 'display:block  !important');
  });

  //.xbox .xbox-type-icon_selector .xbox-search-icon, .xbox .xbox-type-icon_selector .xbox-icons-wrap
})(jQuery);
