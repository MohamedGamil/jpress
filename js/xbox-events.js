XBOX.events = (function (window, document, $) {
  ...
  xbox_events.on_change_image_selector = function ($xbox) {
    $xbox.on('imgSelectorChanged', '.xbox-type-image_selector .xbox-element', function () {
       ...
       */
      var field_id          = $(this).closest('.xbox-type-image_selector').data('field-id');
      var control_img_id    = $(this).closest('.xbox-type-group').find('.xbox-group-control').data('image-field-id');
      var img_src           = $(this).closest('.xbox-item-image-selector').find('img').attr('src');

      
      if(control_img_id == field_id){
        xbox.synchronize_selector_preview_image('.xbox-control-image', img_src, 'add', img_src, control_img_id);
      }
  ...
  xbox_events.show_hide_row = function ($el, field_value, type) {
    ...
    var $id = $el.closest('.xbox-row').data('field-id');
