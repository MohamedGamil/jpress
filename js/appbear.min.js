...
window.APPBEAR = (function (window, document, $) {
      ...
      if (media.multiple) {
        ...
        if (obj.type == 'image') {
          ...
          if (media.field_id == control_img_id) {
            appbear.synchronize_selector_preview_image('.appbear-control-image', media.$wrap_preview, 'add', obj.url);
          }
          //Sincronizar con otros elementos
          appbear.synchronize_selector_preview_image('', media.$wrap_preview, 'add', obj.url);
        }
      }
    });
  ...
  appbear.remove_preview_item = function (event) {
    ...
    if (!multiple && $btn.closest('.appbear-preview-item').hasClass('appbear-preview-image')) {
      if (field_id == control_data_img) {
        appbear.synchronize_selector_preview_image('.appbear-control-image', $wrap_preview, 'remove', '');
      }
      appbear.synchronize_selector_preview_image('', $wrap_preview, 'remove', '');
    }
  ...
  appbear.synchronize_selector_preview_image = function (selectors, $wrap_preview, action, value) {
...
APPBEAR.events = (function (window, document, $) {
  ...
  appbear_events.show_hide_row = function ($el, field_value, type) {
    ...
    var $row_changed = $el.closest('.appbear-row');
    var value = '';
    var operator = '==';
    var $rows = $row_changed.siblings('.appbear-row');
